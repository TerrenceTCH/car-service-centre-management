...
class _VehicleDetailsState extends State<VehicleDetails> {
                                                            ...
                                                            validator: (value) {
                                                              ...
                                                              if(licensePattern.hasMatch(value!)){
                                                                if(vehicleList.isNotEmpty){
                                                                  if(value?.trim().compareTo(widget.vehicle.LicensePlateNumber) != 0){
                                                    ...
                                                    child: FutureBuilder<List<String>>(
                                                      ...
                                                      builder: (context, snapshot){
                                                        if(snapshot.hasData){
                                                          ...
                                                          print('checking vehicle images list${vehicleStatusImages.length}');
                                                        ...
                                                        ElevatedButton(
                                                          onPressed: () async {
                                                            if(updateVehicleFormKey.currentState!.validate()){
                                                              print('im here');
                                                              updateVehicleFormKey.currentState!.save();
                                                              await updateVehicle();
                                                            }
                                                            else{
  ...
  Future<List<String>> loadInitialImages() async{
    ...
    print('im here to check current user id and vehicle id${auth.currentUser?.uid},${widget.vehicle.Uid}');
    List<String> images = [];
  ...
  Future<void> updateVehicle() async{
    ...
    await uploadVehicleImages().catchError((error) {
      print(error.toString());
    });
