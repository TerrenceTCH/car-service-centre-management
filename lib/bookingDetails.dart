...
class bookingDetails extends StatefulWidget {
  ...
  @override
  State<bookingDetails> createState() => _bookingListPageState();
}

class _bookingListPageState extends State<bookingDetails> {
  ...
  @override
  void initState(){
    selectedVehicle = widget.booking.LicensePlateNumber!;
    ...
    serviceDateController.text = dateInString;
                                                      ...
                                                      child: FutureBuilder<List<String>>(
                                                        ...
                                                        builder: (context, snapshot){
                                                          if(snapshot.hasData){
                                                            ...
                                                            print('getting initial selected time ${initialSelectedTime}');
                                                            return Text('Error: ${snapshot.error}');
                                                          } else {
                                                            return Center(child: CircularProgressIndicator());
                                                          }
                                                    ...
                                                    Visibility(
                                                      visible: selectedServiceTimeSlot,
                                                        child: FutureBuilder<List<String>>(
                                                          ...
                                                          builder: (context, snapshot) {
                                                            if(snapshot.hasData){
                                                              print('CHECK INITIAL SELECTED TYPE ${initialSelectedType} in has data');
                                                              ...
                                                              print('CHECK INITIAL SELECTED TYPE ${initialSelectedType} in no data');
                                                              return Text('Error: ${snapshot.error}');
                                                            } else {
                                                              return Center(child: CircularProgressIndicator());
                                                            }
                                                      ...
                                                      child: Row(
                                                        ...
                                                        children: <Widget>[
                                                          ElevatedButton(
                                                            onPressed: () async
                                                            {
                                                              if(updateBookingFormKey.currentState!.validate()){
                                                                print('im here');
                                                                updateBookingFormKey.currentState!.save();
                                                                await updateBooking();
                                                              }
                                                              else{
  ...
  Future<List<bookingObject>> getAllBookingsToList() async {
    ...
    if(snapshot.docs.length>0){
      ...
      dataList = snapshot.docs.map((doc) => bookingObject(doc['ServiceCentreName'], doc['ServiceDate'], doc['LicensePlateNumber'], doc['ServiceType'], doc['Uid'], doc['Status'], doc['TimeSlotUid'], doc['ServiceBayUid'], doc['ServiceCentreUid'], doc['LastUpdatedTime'])).toList();
      if(dataList.length > 0){
        return Future.value(dataList);
      }
    }
  ...
  Future<void> updateBooking() async {
    ...
    print('check isInMinor ${isInMinorOnly}, ${isInMajorAndMinor}, ${isInMajorAndMinorExtra}');
    print('im here to check all the data to be updated ${serviceDateController.text.trim()}  ${selectedTime.trim()} ${Uid} ${selectedTime.trim()}');
    //update booking in user table
    await FirebaseFirestore.instance.collection('users').doc(auth.currentUser?.uid).collection('booking').doc(Uid).update(
      {
        "ServiceCentreName" : serviceCentreSelectedValue,
        "LicensePlateNumber" : selectedVehicle,
        "TimeSlot" : timestamp,
        "ServiceType" : initialSelectedType,
  ...
  Future<void> deleteBooking() async {
    await FirebaseFirestore.instance.collection('users').doc(auth.currentUser?.uid).collection('booking').doc(Uid).delete().catchError((error) {
      print(error);
    });
